#!/bin/bash
# Injects lightweight git context at session start (SessionStart hook).
#
# Default (minimal): branch + dirty/clean indicator. ~5-10 tokens.
# Set CLAUDE_KIT_SESSION_VERBOSE=1 to also emit last commit, changed-file
# count, staged status, stash count, and active PR info. ~30-90 tokens,
# plus a network round-trip if `gh` is installed.

# Bail early if not in a git repo (nothing useful to inject).
git rev-parse --git-dir >/dev/null 2>&1 || exit 0

VERBOSE="${CLAUDE_KIT_SESSION_VERBOSE:-0}"
CONTEXT=""
...
# Verbose extras (opt-in via CLAUDE_KIT_SESSION_VERBOSE=1).
if [ "$VERBOSE" = "1" ]; then
